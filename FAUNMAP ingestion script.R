...
#are there sites with no values in the AGE table?

#FM_LOCALITY[!(FM_LOCALITY$Machine. %in% AGE$Machine..),]

#any duplicated AGE table entries?
#AGE[duplicated(paste0(AGE$Machine..,AGE$AnalysisUnit)),]
...
#Build the Collection Unit XML Block ----------------
  ...
  CUList$CollectionDevice <- DEPOSIT_sub$CollectionDevice
  CUList$DepositionalEnvironment <- DEPOSIT_sub$DepositionalEnvironment
  }
  #Remove NA's
...
#Build the Dataset XML Block -------------------------------------------------------------
  ...
  if(!is.na(Repository_sub)){
    ...
    Rep_df <- as.data.frame(strsplit(Repository_sub, ", ", fixed = TRUE), col.names = "Acronym")
...
#Tilia won't read without the SpreadSheetOptions!
  ...
  distinct_taxa <- FAUNAL_sub %>% select(TaxonName, Modification) %>% distinct
...
#saveXML(XML_out, file="test.TLX")
  ...
  saveXML(XML_out, file = paste0("TLX_Files/",SiteName, ".TLX"), prefix = '<?xml version="1.0"?>\n')
